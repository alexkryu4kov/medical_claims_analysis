...
print(
    f"Amounts: pos={pos:,.2f} | neg={neg:,.2f} | net={net:,.2f} | neg_share={(neg/pos) if pos else np.nan:.2%}"
)
print(
...
by_cat = (
    df.groupby("SERVICE_CATEGORY")["PAID_AMOUNT"].sum().sort_values(ascending=False)
)
monthly_cat = (
    df.groupby(["MONTH", "SERVICE_CATEGORY"])["PAID_AMOUNT"]
    .sum()
