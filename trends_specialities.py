...
def top_specialty_monthly(df: pd.DataFrame,
                          top_n: int = 10,
    ...
    top_specs = (d.groupby("CLAIM_SPECIALTY")[value]
                   .sum().sort_values(ascending=False)
                   .head(top_n).index.tolist())

    # помесячные суммы по специализациям
    pivot = (d.groupby(["MONTH","CLAIM_SPECIALTY"])[value]
               .sum().unstack("CLAIM_SPECIALTY", fill_value=0.0)
               .sort_index())
    ...
    order = (vals.drop(columns=["Other"], errors="ignore")
                 .sum(axis=0).sort_values(ascending=False).index.tolist())
...
monthly_top = top_specialty_monthly(df, top_n=6, value="net", last_n_months=12,
                                    exclude_categories=("PCPEncounter",), drop_unknown=True)
