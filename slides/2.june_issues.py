...
def delta_by_dim(df, dim, m_prev, m_cur, top=15):
    ...
    return out.sort_values("delta").head(top), out.sort_values("delta", ascending=False).head(top)


def volume_price_breakdown(df, dim, m_prev, m_cur, top=15):
    pos = df[df["PAID_AMOUNT"] > 0].copy()
    g = pos[pos["MONTH"].isin([m_prev, m_cur])] \
    ...
    if m_prev not in pivot_sum.columns: pivot_sum[m_prev] = 0
    if m_cur  not in pivot_sum.columns: pivot_sum[m_cur]  = 0
    if m_prev not in pivot_count.columns: pivot_count[m_prev] = 0
    if m_cur  not in pivot_count.columns: pivot_count[m_cur]  = 0
...
for c in focus_cats:
    ...
    by_spec = d.groupby(["MONTH","CLAIM_SPECIALTY"])["PAID_AMOUNT"].sum().unstack(fill_value=0)
    if m1 not in by_spec.index: by_spec.loc[m1] = 0
    if m2 not in by_spec.index: by_spec.loc[m2] = 0
